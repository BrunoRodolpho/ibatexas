...
| ~~**NEW (from H3 Wave A1) — `SupersessionReason` enum is closed**~~ | sibling `@adjudicate/core` | H3 Wave A1 | ✅ READY FOR USER AUTHORIZATION. F2 PR prep landed `"lgpd_scrub"` addition to the union on sibling branch `feat/audit-2026-05-24-additions` (commit `64c013a`). 4-commit bundle ready for `pnpm publish` + `git tag v1.1.0` (user-gated). See `tasks/f2-sibling-readiness-report.md` + sibling's `packages/core/RELEASE-1.1.0.md`. |
| ~~**NEW (from H3 Wave A1) — LoyaltyAccount FK schema constraint**~~ | `LoyaltyAccount.customerId` declared `String @unique` + non-nullable + `Cascade` FK toward `Customer.id` | H3 Wave A1 | ✅ CLOSED at the loyalty-fk-migration follow-up. Schema migrated to `String? @unique` + FK `ON DELETE SET NULL` (`20260524000000_make_loyalty_customer_id_nullable`); H3 anonymize executor now nulls `customerId` in the same `updateMany` that zeroes the counters; T4 reasserts `customerId === null` post-scrub. G2-c (scrub linkage + reset balance + don't delete) now fully delivered. |
| **NEW (from H3 Wave A1) — Reservation.specialRequests non-nullable JSON** | `Reservation.specialRequests` is `Json @default("[]")` not nullable | H3 Wave A1 | A1 set to `[]` (matches schema default; obliterates customer's free-form notes). Schema-investigator pre-flagged this fallback. No semantic loss. |
...
| ~~Code-side stale env-var refs~~ | 3 sites — `tools/src/stripe/adjudicated.ts`, `cli/src/commands/kernel.ts`, `api/src/plugins/kernel-metrics-sink.ts` | M6 → fixed | ✅ CLOSED at `1024028`. Surgical removal of helpers + env-var reads + Prometheus metric registration; `recordShadowDivergence` retained as no-op stub (interface requires the method). 56/56 metrics tests pass. |
| ~~Localized stale refs in load-bearing governance/threat-model docs~~ | 6 sites | M6 → fixed | ✅ CLOSED at `6edd444`. Per-line rewrites with DEPRECATED/HISTORICAL markers; underlying load-bearing content preserved. Line numbers drifted from M6's report; agent self-corrected. |
| ~~**NEW (from stale-machinery cleanup)** — `MetricsSink.recordShadowDivergence` REQUIRED~~ | sibling `@adjudicate/core/kernel` | stale-machinery | ✅ READY FOR USER AUTHORIZATION. F2 PR prep relaxed method to optional + threaded `?.()` safety guards through framework call sites (`setMetricsSink` shadow-telemetry branches + `MetricsSinkSlot` wrapper) on sibling branch `feat/audit-2026-05-24-additions` (commit `c612104`). User-gated publish. |
| ~~**NEW (from stale-machinery cleanup)** — `docs/ops/analytics-dashboards.md` stale rows~~ | `docs/ops/analytics-dashboards.md` (5 sites) | stale-machinery | ✅ CLOSED at `bcac0d4`. 5 sites updated with HISTORICAL banners; rollout-dashboards section collapsed; PostHog shadow-divergence query removed; kernel metrics table tightened to 5 actively-emitted events. |
| ~~**NEW (from analytics-dashboards cleanup)** — orphan `audit_kernel_shadow_diverged_*` union members~~ | `apps/web/src/domains/analytics/events.ts:107-109` | analytics-dashboards (flagged), closed inline | ✅ CLOSED at `85cf249`. 3 orphan union members removed; section comment updated to drop the `(v2.0)` qualifier; apps/web typecheck clean; no consumer references in apps/ or packages/. |
| **NEW (from F2 PR prep)** — `SupersessionReason` extension cascade | sibling `packages/core/src/explain.ts`, `audit/supersession-chain.ts`, `admin-sdk/audit.ts`, `console/{LineageGraph,SupersessionChain}.tsx` | F2 PR prep | Adding `"lgpd_scrub"` to the union cascaded into 4 exhaustive-enum sites + 1 test fixture. F2 agent correctly folded all into commit `64c013a`. Worth a sibling-side conformance test that asserts every `Record<SupersessionReason, ...>` site is exhaustive at compile-time — future work. |

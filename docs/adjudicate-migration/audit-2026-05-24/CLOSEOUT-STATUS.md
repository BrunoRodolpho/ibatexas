...
| **NEW (from H3 Wave A1) — `SupersessionReason` enum is closed** | `@adjudicate/core` `SupersessionReason` union: `"confirmation_resolved" \| "defer_resumed" \| "rewrite_executed" \| "replay"` | H3 Wave A1 | A1 wanted `"lgpd_scrub_extension"` for per-surface audit records; chose `"replay"` as closest semantic. Adding `"lgpd_scrub"` to the union would belong in the next `@adjudicate/core` minor — small cross-repo PR. |
| **NEW (from H3 Wave A1) — LoyaltyAccount FK schema constraint** | `LoyaltyAccount.customerId` declared `String @unique` + non-nullable + `Cascade` FK toward `Customer.id` | H3 Wave A1 | G2-c said "scrub linkage + reset balance + don't delete". A1 reset the counters (stamps/totalEarned/redeemed — the schema's actual balance surface; no `balance` column) but kept `customerId` pointing at the already-anonymized Customer row (FK constraint requires it). True linkage scrubbing would need a schema migration to make `customerId` nullable. Effectively-equivalent today since the Customer row itself is anonymized. |
...
| **NEW (from stale-machinery cleanup)** | `@adjudicate/core/kernel.MetricsSink.recordShadowDivergence` declared as REQUIRED method | stale-machinery (out-of-scope flag) | The IbateXas sink had to keep a no-op stub because the interface requires the method. Sibling repo could relax it to optional in the next minor — bundle with F2 (`KERNEL_INTENT_DISPATCHED` + `"lgpd_scrub"`)? Tracked as F2 sub-item. |
| **NEW (from stale-machinery cleanup)** | `docs/ops/analytics-dashboards.md` rows 329-330, 340, 343, 354, 385 | stale-machinery (out-of-scope flag) | Doc references `kernel_shadow_divergence_total` metric (now deleted). Markdown doc, not an executable query — no live dashboard rendering breaks. Worth aligning. ~15min. |

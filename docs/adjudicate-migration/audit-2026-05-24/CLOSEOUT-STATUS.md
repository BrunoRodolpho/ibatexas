...
|---|---|---|---|
| Migrate gateway callers from `buildEnvelope` to `buildCustomerEnvelope` | `apps/api/src/routes/{cart,me,order-actions}.ts` (8+ sites) | Polish-B | Closes the "wider-type back-door" so the structural defense is total. Mechanical sweep, ~1-2h. *In flight in current burst.* |
| Stale `IBX_KERNEL_SHADOW` / `IBX_KERNEL_ENFORCE` env-var stubs | `apps/api/src/__tests__/cart-routes.test.ts` SEC-001 suite (3 failing tests) | Polish-B | Tests pre-date the cutover; the kernel no longer reads those env vars. *In flight in current burst.* |

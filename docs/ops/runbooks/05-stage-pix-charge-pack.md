...
The Pack's PolicyBundle covers all six kernel Decision outcomes; coverage is exercised in [packages/pack-payments-pix/tests/decisions.test.ts](../../../packages/pack-payments-pix/tests/decisions.test.ts) and a DEFER round-trip test in [packages/pack-payments-pix/tests/defer-round-trip.test.ts](../../../packages/pack-payments-pix/tests/defer-round-trip.test.ts).
...
- [ ] **Pack version pinned** in `packages/llm-provider/package.json` and `apps/api/package.json` (no `workspace:*` floats during rollout — pin to a concrete version)
...
- File `docs/ops/postmortems/stage-5-pix-charge-pack-rollout.md` with:
  ...
  - Pack version migration path (`workspace:*` → pinned `0.1.0-experimental` → eventual `0.x.y` from npm)

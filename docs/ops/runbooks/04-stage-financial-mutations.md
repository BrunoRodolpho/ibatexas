...
DEFER + webhook resume path (`apps/api/src/subscribers/defer-resolver.ts` → `@ibx/intent-runtime resumeDeferredIntent`) exercises here. Ledger SET-NX dedup prevents double-execution from duplicate webhook deliveries.
...
- [ ] **Ledger flags staged:**
  ...
  - [ ] Postgres audit sink (`@ibx/intent-audit-postgres`) writing without backlog (lag <30s)

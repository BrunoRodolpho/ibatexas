...
9. **LLM Authority (IBX Intent-Gated Execution v2.0):** the LLM is a semantic parser with zero state-mutation authority. Mutating tools are captured as `IntentEnvelope<kind, payload>` objects (`@adjudicate/core`), adjudicated by the kernel (`adjudicate()` from `@adjudicate/core/kernel`), and orchestrated via `@ibatexas/llm-provider` (`runOrchestrator`, `executeKernel`) which composes deadline helpers from `@adjudicate/runtime`. Tool visibility to the LLM is controlled by the `CapabilityPlanner` and `ToolClassification` contracts in `@adjudicate/core/llm`; never add mutating tools to the planner's visible list. Current implementations: `packages/llm-provider/src/capability-planner.ts`, `TOOL_CLASSIFICATION` in `packages/llm-provider/src/machine/types.ts`, intent-bridge in `packages/llm-provider/src/tool-registry.ts`. **Staged rollout** is gated per-intent-class via `IBX_KERNEL_SHADOW` and `IBX_KERNEL_ENFORCE` (parsed in `@adjudicate/core/kernel`'s `enforce-config`) — see [docs/ops/runbooks/](docs/ops/runbooks/) for the 4-stage playbook. **Execution dedup** is gated by `IBX_LEDGER_ENABLED` / `IBX_LEDGER_ENFORCE` / `IBX_LEDGER_FAIL_OPEN` (`@adjudicate/audit`). **PIX charge lifecycle** lives in `@adjudicate/pack-payments-pix` (the lighthouse Pack, source: [BrunoRodolpho/adjudicate](https://github.com/BrunoRodolpho/adjudicate)). IbateXas's `order-policy-bundle.ts` composes the Pack's `createPixPendingDeferGuard` factory; the inline `PIX_*` constants and `deferOnPendingPix` guard were deleted in the migration. New PIX-pending consumers must import constants and the guard factory from `@adjudicate/pack-payments-pix`, never re-declare them. **Source-of-truth for `@adjudicate/*` packages is the platform repo** — IbateXas's `pnpm-workspace.yaml` includes `../adjudicate/packages/*` so `workspace:*` deps resolve cross-repo until npm publication lands. See ADRs #9 and #13 and the platform repo's package READMEs.

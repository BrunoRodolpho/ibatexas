...
| Turn pipeline — perceive → plan → adjudicate → dispatch → reply | [docs/architecture/design/turn-pipeline.md](docs/architecture/design/turn-pipeline.md) |
...
9. **LLM Authority:** the LLM is a semantic parser with zero state-mutation authority. A turn flows `express_intent(capability, payload)` → the claustrum **Conductor** → `capsule.adjudicator` → the **`@adjudicate/*` kernel**, which decides `EXECUTE` / `REFUSE` / `REQUEST_CONFIRMATION` / … — and only an `EXECUTE` runs the mutation (via the wrapper's `onExecute` callback). Mutating authority lives in the kernel via the PolicyBundles in `packages/pack-*`; never give the LLM a path to mutate state directly.
10. **Redis locks:** always use UUID lock values with Lua conditional release. Never use plain `redis.del()` to release a lock — use the ownership-checking Lua script pattern. See `apps/api/src/streaming/execution-queue.ts` for reference.

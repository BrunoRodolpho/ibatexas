...
9. **LLM Authority:** the LLM is a semantic parser with zero state-mutation authority. Mutating tools use `executeToolDirect()` (kernel-only). Never add mutating tools to `STATE_TOOLS` in the prompt synthesizer — they are kernel-controlled via `TOOL_CLASSIFICATION` in `packages/llm-provider/src/machine/types.ts`. _(Cutover caveat — audit D-CLAUDEMD9: this describes the **legacy** `@ibatexas/llm-provider` brain, which is still the live path. The in-progress migration replaces it with the `@claustrum/*` capability model — `express_intent(capability, payload)` → `capsule.adjudicator` → `@adjudicate/*` kernel — but that runtime is INERT today (RC-A1 activation deferred to remediation cycle 4). Until activation, this rule governs; post-activation, mutating authority is the kernel's via the PolicyBundles in `packages/pack-*`.)_

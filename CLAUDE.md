...
9. **LLM Authority (IBX Intent-Gated Execution v2.0):** the LLM is a semantic parser with zero state-mutation authority. Mutating tools are captured as `IntentEnvelope<kind, payload>` objects (`@adjudicate/intent-core`), adjudicated by the kernel (`@adjudicate/intent-kernel.adjudicate()`), and orchestrated via `@adjudicate/intent-runtime` (`runOrchestrator`, `executeKernel`). Tool visibility to the LLM is controlled by the `CapabilityPlanner` and `ToolClassification` contracts in `@adjudicate/intent-llm`; never add mutating tools to the planner's visible list. Current implementations: `packages/llm-provider/src/capability-planner.ts`, `TOOL_CLASSIFICATION` in `packages/llm-provider/src/machine/types.ts`, intent-bridge in `packages/llm-provider/src/tool-registry.ts`. **Staged rollout** is gated per-intent-class via `IBX_KERNEL_SHADOW` and `IBX_KERNEL_ENFORCE` (parsed in `packages/llm-provider/src/intent-enforce-config.ts`) — see [docs/ops/runbooks/](docs/ops/runbooks/) for the 4-stage playbook. **Execution dedup** is gated by `IBX_LEDGER_ENABLED` / `IBX_LEDGER_ENFORCE` / `IBX_LEDGER_FAIL_OPEN` (`@adjudicate/intent-audit`). See ADR #9 and the `@adjudicate/intent-{core,kernel,llm,runtime,audit,audit-postgres}` package READMEs.

    ...
    <>
      {showRating && (
        <div className="mt-1.5 inline-flex items-center gap-1">
          ...
          <span className="text-[11px] text-[var(--color-text-secondary)]">({reviewCount})</span>
        </div>
      )}
      {tags?.includes('popular') && reviewCount && reviewCount > 50 && (
        <p className="text-[11px] text-[var(--color-text-secondary)]">{t('product.ordered_count', { count: reviewCount })}</p>
      )}

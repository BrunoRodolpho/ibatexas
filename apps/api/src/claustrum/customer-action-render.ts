 ...
 */
/**
 * LE2-021 — the AUTHORED workflow outcome sentence carried on a dispatch result,
 * or `undefined` when this action was not a workflow run.
 *
 * `installWorkflowRuntime`'s anchor wrapper stamps `{message, workflow:{…}}` onto
 * the result, where `message` is `renderOutcome(instance, outcome)` — the
 * workflow's own authored pt-BR template for the terminal it actually reached.
 * The `workflow` key is the discriminator: it is written only by that wrapper,
 * and only after a run, so its presence is proof the message is authored catalog
 * text rather than an executor's free-form string.
 */
function workflowOutcomeMessage(result: unknown): string | undefined {
  if (!isRecord(result) || !isRecord(result.workflow)) return undefined;
 ...
 *
 * A workflow's authored outcome template is checked FIRST and returned VERBATIM,
 * because without that branch the comment in `workflow-install.ts` — "the reply
 * the customer reads comes from the AUTHORED template … never from model prose"
 * — was simply false, in both of the two ways it could be:
 *
 *   - For an anchor this module already renders (the LE2-020 fixture's
 *     `order.checkout.create`), `renderCheckoutCreate` reads only
 *     `result.success`/`result.paymentMethod` and DROPS `result.message`. The
 *     customer would be told a PIX code was generated by a run whose authored
 *     outcome was "Não consegui concluir todas as etapas".
 *   - For an anchor it does not render (`order.reorder.request`), this function
 *     returns `undefined`, the responder falls through to its grounded model
 *     branch, and `summarizeActed` forwards `result` — so the authored sentence
 *     reaches the MODEL as context and comes back PARAPHRASED. Model prose,
 *     which the renderer-from-claims design forbids outright.
 *
 * Both are silent: the reply is fluent and plausible either way. So the workflow
 * branch is unconditional and returns before any per-kind template runs — a run
 * that reached a terminal has exactly one honest sentence, and the catalog
 * authored it.
 */
export function renderCustomerActionAnswer(acted: unknown): string | undefined {
  const actions = executedOpsActions(acted);

  const workflowLines = actions
    .map((a) => workflowOutcomeMessage(a.result))
    .filter((line): line is string => line !== undefined);
  if (workflowLines.length > 0) return workflowLines.join("\n\n");

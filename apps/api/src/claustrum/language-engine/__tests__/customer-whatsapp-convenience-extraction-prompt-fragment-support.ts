// customer-whatsapp-convenience-extraction-prompt-fragment-support.ts —
// TEST SUPPORT (not a *.test.ts, so vitest never collects it directly —
// mirrors order-cart-item-extraction-prompt-fragment-support.ts's
// convention).
//
// FE-T14 — computes the FRESH composed `express_intent` tool JSON for the
// pack-customer-onboarding + pack-whatsapp family (customer.preferences.
// update, customer.pix.details.save, whatsapp.handoff.request) when all
// three are simultaneously in the allowed-intent set.
//
...
export interface CustomerWhatsappConvenienceExtractionPromptFragment {
...
const CUSTOMER_WHATSAPP_CONVENIENCE_CAPABILITIES = [
...
function mkState(text: string): CognitiveState {
  return {
    ...
    tenantId: "ibatexas",
    locale: "pt-BR",
    conversationId: "conv-golden-customer-whatsapp",
    turnId: "turn-golden-customer-whatsapp",
...
/**
 * Compute the FRESH extraction-prompt fragment for the customer/whatsapp
 * group by driving the REAL `createIbatexasPlanner` with a capability
 * planner that allows all three kinds and capturing the `express_intent`
 * tool the model would actually receive.
 */
export async function computeCustomerWhatsappConvenienceExtractionPromptFragment(): Promise<CustomerWhatsappConvenienceExtractionPromptFragment> {
  const { model, calls } = noToolCallModel();
  const planner = createIbatexasPlanner({
    model,
    modelId: "claude-test",
    capabilityPlanners: [capPlanner([...CUSTOMER_WHATSAPP_CONVENIENCE_CAPABILITIES])],
  });
  ...
  if (tool === undefined) {
    throw new Error("extraction-prompt golden (customer-whatsapp-convenience): express_intent tool missing from buildToolSurface output");
  }

  return {
    capabilities: CUSTOMER_WHATSAPP_CONVENIENCE_CAPABILITIES,
    expressIntentTool: {
      name: tool.name,

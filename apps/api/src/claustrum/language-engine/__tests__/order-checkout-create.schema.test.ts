...
// authoring-contract lint and exposes ONLY `paymentMethod` — never cartId
...
describe("ORDER_CHECKOUT_CREATE_EXTRACTION_SCHEMA", () => {
  ...
  it("exposes ONLY paymentMethod — never cartId, pixDetails, email, or cpf", () => {
    const names = extractionFieldNames(ORDER_CHECKOUT_CREATE_EXTRACTION_SCHEMA);
    expect([...names]).toEqual(["paymentMethod"]);
  ...
  it("the worked example matches the schema's fields and carries no forbidden name", () => {
    ...
    expect(payload).toEqual({ paymentMethod: "pix" });

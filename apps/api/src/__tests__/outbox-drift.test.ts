...
// how payment.status_changed regressed). An event persisted but not retried is never
// re-published on broker recovery (the customer.anonymize.medusa.pending drift).
// CRITICAL_EVENTS now DERIVES from OUTBOX_EVENTS; this test pins that invariant.

import { describe, it, expect, vi } from "vitest";

// outbox-retry imports @ibatexas/tools (getRedisClient/rk), whose dist pulls the
// domain chain; mock it so this test collects.
...
describe("outbox durability set drift guard (P0-PAY-4)", () => {
  ...
  it("includes customer.anonymize.medusa.pending in BOTH sets (no persisted-but-not-retried gap)", () => {
    expect(OUTBOX_EVENTS.has("customer.anonymize.medusa.pending")).toBe(true);
    expect(CRITICAL_EVENTS).toContain("customer.anonymize.medusa.pending");
  });
});

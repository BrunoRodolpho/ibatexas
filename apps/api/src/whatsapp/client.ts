...
/**
 * Core sender shared by sendText/sendMedia. Routes through the proactive
 * rate limiter and a client-side idempotency guard, then retries ONLY on
 ...
 * @param logTag  Log namespace ("send" | "sendMedia").
 */
async function createMessage(
  ...
  if (!(await claimIdempotencyKey(idemKey))) {
    logger.warn({ phone_hash: hash, part }, `[whatsapp.${logTag}.deduped]`);
    return;
  }
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (isPostSendAmbiguous(err)) {
        logger.warn({ phone_hash: hash }, `[whatsapp.${logTag}.timeout_no_retry]`);
        throw err;
      }
...
async function sendSingleMessage(
  ...
  await createMessage({ from: getWhatsAppNumber(), to, body }, hash, part, "send");
}

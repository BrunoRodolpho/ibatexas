...
|---|---|---|---|
| **Read** = Access ⊕ Provenance | *may I read it? + what's its trust?* | P1 ownership + **PII-minimization + tenant-isolation** + provenance | access: `ALLOW_READ`·`REDACT`·`ESCALATE`·`REFUSE`; provenance: `TRUSTED`/`UNTRUSTED_DATA` |
...
{ key, value, source,
  ...
  originProvenance,                  // survives persistence — a row written from UNTRUSTED ingress stays UNTRUSTED
...
3. **Provenance persists and gates values** — `UNTRUSTED_DATA` may never be the *validating value* of any claim; persisted state carries its origin's provenance.
...
**Size (transcribe exactly): 37 rows / 40 type names** (3 rows are slash-paired existence pairs). The verified count is **37**, not 33 (the uncorrected figure in registry v0.1, fixed this session) and not 28 (the older v0 count) — see Section P.
...
- ❌ "33 claim types" (or the older "28"). → ✅ **37 rows / 40 type names** (Section K). 33 was the uncorrected v0.1 figure; 28 was the v0 count.

...
function TableSkeleton({ columns = 5, rows = 5 }: Readonly<{ columns?: number; rows?: number }>) {
  const headerKeys = Array.from({ length: columns }, (_, i) => `th-${i}`)
  const rowKeys = Array.from({ length: rows }, (_, i) => `tr-${i}`)
  const cellKeys = Array.from({ length: columns }, (_, i) => `td-${i}`)
  return (
...
function StatsSkeleton({ count = 4 }: Readonly<{ count?: number }>) {
  const cardKeys = Array.from({ length: count }, (_, i) => `stat-${i}`)
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      {cardKeys.map((key) => (
        <div key={key} className="rounded-sm border border-smoke-200 border-l-2 border-l-smoke-300 bg-smoke-50 p-5">
...
function ListSkeleton({ rows = 5 }: Readonly<{ rows?: number }>) {
  const rowKeys = Array.from({ length: rows }, (_, i) => `list-${i}`)
  return (
    <div className="space-y-2">
      {rowKeys.map((key) => (
        <div key={key} className="h-14 animate-pulse rounded-sm border border-smoke-200 bg-smoke-100" />
      ))}
    </div>
  )
}

# @ibx/intent-audit-postgres

Postgres durable governance trail for IBX Intent-Gated Execution.

Implements `AuditSink` from `@ibx/intent-audit` against a partitioned-by-month
...
```ts
import { createPostgresSink } from "@ibx/intent-audit-postgres";
import { multiSink, createConsoleSink, createNatsSink } from "@ibx/intent-audit";
...
```ts
import { readAuditWindow } from "@ibx/intent-audit-postgres";
...
`@ibx/intent-audit` stays free of Postgres-specific imports — adopters that

...
import type { PaymentStatus, PaymentMethod } from "./payment-status.js"
...
// ── §4.1 Cancel Order ──────────────────────────────────────────────
function checkCancelOrder(ctx: ActionContext): ActionResult {
...
// ── §4.2 Add Item ──────────────────────────────────────────────────
function checkAddItem(ctx: ActionContext): ActionResult {
  const fs = ctx.fulfillmentStatus
...
function checkRemoveOrUpdateQty(ctx: ActionContext): ActionResult {
  const fs = ctx.fulfillmentStatus
  if (fs === "canceled") return deny("Pedido cancelado.")
  if (fs === "delivered") return deny("Pedido já entregue.")
...
function checkChangePaymentMethod(ctx: ActionContext): ActionResult {
...
// ── §4.5 Retry Payment ─────────────────────────────────────────────
function checkRetryPayment(ctx: ActionContext): ActionResult {
  const ps = ctx.paymentStatus
  if (!ps) return deny("Status de pagamento desconhecido.")
...
// ── §4.6 Regenerate PIX ────────────────────────────────────────────
function checkRegeneratePix(ctx: ActionContext): ActionResult {
  const ps = ctx.paymentStatus
  if (!ps) return deny("Status de pagamento desconhecido.")
...
// ── §4.7 Add Notes ─────────────────────────────────────────────────
function checkAddNotes(ctx: ActionContext): ActionResult {
  if (ctx.fulfillmentStatus === "canceled") return deny("Pedido cancelado — não pode adicionar observações.")
  return ALLOWED
}

// ── §4.8 Change Delivery Address ───────────────────────────────────
function checkChangeDeliveryAddress(ctx: ActionContext): ActionResult {
  const fs = ctx.fulfillmentStatus
...
function checkSwitchOrderType(ctx: ActionContext): ActionResult {
  const fs = ctx.fulfillmentStatus
  if (fs === "canceled") return deny("Pedido cancelado.")
  if (fs === "delivered") return deny("Pedido já entregue.")
  if (fs !== "pending") {
...
/** Dispatch table — maps each customer action to its validator. */
const ACTION_VALIDATORS: Record<CustomerAction, (ctx: ActionContext) => ActionResult> = {
  cancel_order: checkCancelOrder,
  amend_add_item: checkAddItem,
  amend_remove_item: checkRemoveOrUpdateQty,
  amend_update_qty: checkRemoveOrUpdateQty,
  change_payment_method: checkChangePaymentMethod,
  retry_payment: checkRetryPayment,
  regenerate_pix: checkRegeneratePix,
  add_notes: checkAddNotes,
  change_delivery_address: checkChangeDeliveryAddress,
  switch_order_type: checkSwitchOrderType,
}
...
export function canPerformAction(action: CustomerAction, ctx: ActionContext): ActionResult {
  ...
  if (!validate) return deny("Ação não reconhecida.")
  return validate(ctx)
}

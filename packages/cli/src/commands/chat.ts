...
// NOTE (claustrum-on-dev WS8): the former `chat scenarios` subcommand and
// its `runScenarios` runner were removed here. Their E2E harness lived in
// `packages/llm-provider/src/__tests__/scenarios/` — legacy-brain test
// fixtures + a scenario-runner that imported `@ibatexas/llm-provider`
// internals — and was deleted with that package. No replacement exists, so
// the command had no subject. (See migration decision [D1].)

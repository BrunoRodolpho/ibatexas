...
export function registerStripeCommands(group: Command): void {
  ...
  group
    ...
    .action(async () => {
      ...
      const publishableKey = validateKey("NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY", process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY, "pk_")
      ...
      console.log(`  ${icon(publishableKey.ok)} NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY ${chalk.gray(publishableKey.detail)}`)
  ...
  // ─── stripe complete ────────────────────────────────────────────────────
  // Dev-only rescue: completes orphaned carts whose Stripe webhook never
  // fired (no `stripe listen` running), forcing the Medusa order to be
  // created so it shows up in /admin/pedidos and /account/orders.
  group
    .command("complete")
    .description("Force-complete orphaned PIX/card carts (dev rescue when no webhook fired)")
    .option("--cart <cartId>", "Complete a specific cart by ID")
    .option("--all", "Complete every cart tracked in active:carts")
    .action(async (opts: { cart?: string; all?: boolean }) => {
      if (process.env.NODE_ENV === "production") {
        console.error(chalk.red("\n  Refusing to run in production — this is a dev rescue tool.\n"))
        process.exit(1)
      }

      if (!opts.cart && !opts.all) {
        console.error(chalk.red("\n  Pass --cart <cartId> or --all.\n"))
        process.exit(1)
      }

      const { medusaStore } = await import("@ibatexas/tools")

      async function completeOne(cartId: string): Promise<{ ok: boolean; orderId?: string; error?: string }> {
        try {
          const data = await medusaStore(`/store/carts/${cartId}/complete`, {
            method: "POST",
            body: JSON.stringify({}),
          }) as { type?: string; order?: { id: string; display_id?: number } }
          const orderId = data.order?.display_id
            ? formatOrderId(data.order.display_id)
            : data.order?.id
          if (orderId) {
            // Untrack from active:carts so abandoned-cart checker stops polling
            try {
              const r = await getRedis()
              await r.hDel(rk("active:carts"), cartId)
            } catch {
              // Redis unavailable — non-critical
            }
            return { ok: true, orderId }
          }
          return { ok: false, error: `no order in response (type=${data.type ?? "?"})` }
        } catch (err) {
      ...
      // Single cart mode
      if (opts.cart) {
        const spinner = ora({ text: `Completing cart ${opts.cart}…`, indent: 2 }).start()
        const result = await completeOne(opts.cart)
        if (result.ok) {
          spinner.succeed(chalk.green(`Order created: ${result.orderId}`))
        } else {
      ...
      // --all mode: scan active:carts hash
      const spinner = ora("Scanning active:carts…").start()
      let cartIds: string[] = []
      try {
        const r = await getRedis()
        const entries = await r.hGetAll(rk("active:carts"))
        cartIds = Object.keys(entries)
      } catch (err) {
        spinner.fail(chalk.red(`Redis error: ${(err as Error).message}`))
        process.exit(1)
      }

      if (cartIds.length === 0) {
        spinner.info(chalk.gray("No carts in active:carts — nothing to do."))
        await closeRedis()
        return
      }

      spinner.stop()
      console.log(chalk.bold(`\n  Found ${cartIds.length} tracked cart(s). Completing each…\n`))

      let succeeded = 0
      let failed = 0
      for (const cartId of cartIds) {
        const result = await completeOne(cartId)
        if (result.ok) {
          succeeded++

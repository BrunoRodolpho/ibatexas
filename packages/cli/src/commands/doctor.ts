...
export function registerDoctorCommands(program: Command): void {
  program
    ...
    .action(async (opts: { fix?: boolean; ci?: boolean }) => {
      ...
      await runInfraCheck(results, "Postgres", async () => {
        try {
      ...
      await runInfraCheck(results, "Redis", async () => {
        try {
      ...
      await runInfraCheck(results, "Typesense", async () => {
        try {
      ...
      // ── 2. Data Integrity ─────────────────────────────────────────────
      console.log(chalk.bold("\n  Data Integrity"))
      await runInfraCheck(results, "Products", async () => {
        try {
      ...
      await runInfraCheck(results, "Reviews", async () => {
        try {
          const { prisma } = await import("@ibatexas/domain")
      ...
      await runInfraCheck(results, "Order Items", async () => {
        try {
          const { prisma } = await import("@ibatexas/domain")
      ...
      // ── 4. UI Contracts ───────────────────────────────────────────────
      console.log(chalk.bold("\n  UI Contracts"))
      await runInfraCheck(results, "Popular Tags", async () => {
        try {
          const products = await fetchAllProductsWithTags()
          const count = products.filter((p) => p.tags?.some((t) => t.value === "popular")).length
          return { status: count > 0 ? "ok" : "warn", detail: `${count} products tagged popular` }
        } catch {
          return { status: "warn", detail: "Medusa unavailable" }
        }
      })
      await runInfraCheck(results, "Chef Choice Tags", async () => {
        try {
          const products = await fetchAllProductsWithTags()
          const count = products.filter((p) => p.tags?.some((t) => t.value === "chef_choice")).length
          return { status: count > 0 ? "ok" : "warn", detail: `${count} products tagged chef_choice` }
        } catch {
          return { status: "warn", detail: "Medusa unavailable" }
        }
      })

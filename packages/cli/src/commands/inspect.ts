  ...
  group
    ...
    .action(async (handle: string) => {
      ...
      try {
        ...
        // Typesense data (rating, price)
        try {
          const { getTypesenseClient, COLLECTION } = await import("@ibatexas/tools")
          const ts = getTypesenseClient()
          const doc = await ts.collections(COLLECTION).documents(product.id).retrieve() as Record<string, unknown>
        ...
        try {
          ...
          // Copurchase
          const members = await redis.zRangeWithScores(rk(`copurchase:${product.id}`), 0, 9, { REV: true })
          if (members.length > 0) {
            console.log(chalk.bold("\n  Copurchase"))
  ...
  group
    ...
    .action(async () => {
      ...
      try {
        ...
        if (sorted.length > 0) {
          // Resolve top product names from Medusa

...
export async function getEmbeddingCache(productId: string): Promise<number[] | null> {
  try {
    ...
    const key = `product_embedding:${productId}`
...
export async function batchSetEmbeddingCache(
  ...
  try {
    ...
    for (const embedding of embeddings) {
      pipeline.setEx(`product_embedding:${embedding.productId}`, ttl, JSON.stringify(embedding.embedding))
    }
    await pipeline.exec()
...
export async function clearEmbeddingCache(): Promise<number> {
  try {
    ...
    for await (const key of redisClient.scanIterator({ MATCH: "product_embedding:*", COUNT: 100 })) {
      keys.push(key)
    }

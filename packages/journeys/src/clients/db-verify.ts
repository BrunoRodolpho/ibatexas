...
async function placeOrder(qty: number, known: Set<string>, ctx: Ctx): Promise<string | null> {
  const cart = (await ctx.http("POST", "/api/cart", {})).json?.cart?.id
  if (!cart) return null
  await ctx.http("POST", `/api/cart/${encodeURIComponent(cart)}/line-items`, { variant_id: ctx.variant.id, quantity: qty })
  const co = await ctx.http("POST", "/api/cart/checkout", { cartId: cart, paymentMethod: "cash", deliveryType: "pickup" })
  if (co.status >= 400) { say(`  (checkout failed HTTP ${co.status}: ${oneLine(JSON.stringify(co.json).slice(0, 160))})`); return null }
  // wait for a NEW projected order id distinct from any already seen (tolerate projector lag)
  const dl = Date.now() + 20000
  for (;;) {
    const o = await ctx.domain.latestOrderForCustomer(ctx.customer.id)
...
async function createKnownState(ctx: Ctx): Promise<{ pendingId: string | null; canceledId: string | null }> {
  ...
  const pendingId = await placeOrder(2, known, ctx)
  say(`  order#1 (pending target): ${pendingId ?? "(failed)"}`)
  await new Promise((r) => setTimeout(r, 2000))
  const canceledId = await placeOrder(3, known, ctx)
  ...
  if (canceledId) {
    const c = await ctx.http("POST", `/api/orders/${encodeURIComponent(canceledId)}/cancel`, { reason: "verificação de estado" })
    say(`  cancel order#2 → HTTP ${c.status}`)
    // wait for projection to flip
    const dl = Date.now() + 12000
    for (;;) { const o = await ctx.domain.latestOrderForCustomer(ctx.customer.id); if (o?.fulfillmentStatus === "canceled" || Date.now() >= dl) { break } await new Promise((r) => setTimeout(r, 800)) }
  }
  return { pendingId, canceledId }
}

async function censusSection(ctx: Ctx): Promise<string | undefined> {
  const { q } = ctx
  // ── Schema census ───────────────────────────────────────────────────────
...
async function seededEntities(ctx: Ctx): Promise<void> {
  const { q } = ctx
...
async function verifyOrderPending(ctx: Ctx, pendingId: string | null): Promise<void> {
  const { q } = ctx
...
async function verifyOrderCanceled(ctx: Ctx, canceledId: string | null): Promise<void> {
  const { q } = ctx
...
async function checkHashChain(q: QueryFn, auditSchema: string, cols: string[]): Promise<void> {
  ...
  say(`  (hash-chain columns not both found: hash=${hashCol ?? "?"} prev=${prevCol ?? "?"} — tamper-evidence separately proven by journeys audit.record.verified)`)
}

async function auditTrail(ctx: Ctx, auditSchema: string | undefined): Promise<void> {
  const { q } = ctx
...
async function writeReport(pool: pg.Pool, pendingId: string | null, canceledId: string | null): Promise<number> {
  // ── Report ──────────────────────────────────────────────────────────────
...
async function main(): Promise<void> {
  ...
  const ctx: Ctx = { q, domain, http: (m, p, body) => http(m, p, cookie, body), customer, variant }
  ...
  const { pendingId, canceledId } = await createKnownState(ctx)
  const auditSchema = await censusSection(ctx)
  await seededEntities(ctx)
  await verifyOrderPending(ctx, pendingId)
  await verifyOrderCanceled(ctx, canceledId)
  await auditTrail(ctx, auditSchema)

  const failCount = await writeReport(pool, pendingId, canceledId)
